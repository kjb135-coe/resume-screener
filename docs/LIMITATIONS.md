...
**The realistic path is a hybrid**: real resume text from a public
dataset, re-labelled against one posting by a person doing the hiring.
Even 30 of those would be worth more than 60 synthetic ones, because the
text would stop being generated by the same kind of model that scores it.
